--author : chenzutao
--date :   2014-07-07
--func :   decode URL info, and then get GPS info , GSENSOR info
...
local function gmttime_to_time(gmttime)
        return os.time({day=tonumber(string.sub(gmttime, 1, 2)), month=tonumber(string.sub(gmttime, 3, 4)), year=tonumber(string.sub(os.date('%Y'), 1, 2)*100) + tonumber(string.sub(gmttime, 5, 6)), hour=tonumber(string.sub(gmttime, 7, 8)), min=tonumber(string.sub(gmttime, 9, 10)), sec=tonumber(string.sub(gmttime, 11, 12))})
end
...
local function get_users(tim)
	...
	if not ok then
		--print('failed to get users from redis -> ' .. tim)
...
local function decode_gps(gps, v_tab, tim, ext)
	...
	for i=1, #gps_tab do
		...
		--print('GMTtime ----->> ' .. tostring(GMTtime))	
		if (type(tonumber(GMTtime)) ~= 'number') or string.len(GMTtime) ~= 12 then
			--print(string.format('GMTtime -->>[decode_gps]->%s type is invalid !', GMTtime))
			only.log('E', string.format('GMTtime error: -->>[decode_gps]->GMTtime ->%s<- type is invalid ,imei->%s<-, timestamp:%s, exsit time: %s!', GMTtime, v_tab['imei'], tim, os.date('%Y-%m-%d %H:%M:%S', ext)))
			--return nil
			do break end
		end
...
local function decode_gsensor(gsensor, v_tab, tim, ext)
	...
	if type(GMTtime_st) == 'number' and type(GMTtime_ed) == 'number' and string.len(GMTtime) == 11 then
        ...
        if (type(tonumber(GMTtime_st)) ~= 'number') or type(tonumber(GMTtime_ed)) ~= 'number' or string.len(GMTtime_st) ~= 12 or string.len(GMTtime_ed) ~= 12 then
                --print(string.format('GMTtime ---->>[decode_gsensor]->%s type is invalid !', GMTtime))
...
local function get_url(imei, tim)
	...
	if not ok then
		--print(string.format('failed to get url --> imei:%s, time:%s', imei, tim))
		only.log('E', string.format('failed to get url --> imei:%s, time:%s', imei, tim))
		return nil
	end
	if not ret or #ret == 0 then
		--print(string.format('imei:%s, time:%s --> have no url', imei, tim))
...
--	local st_date = '2014-06-17 00:00:00'
--	local ed_date = '2014-06-17 08:00:00'
	...
	for i=unix_time_st, unix_time_ed, 600 do
		...
		for j=1, #users do
			...
			if urls and #urls>0 then
				...
				--print(string.format('imei:%s, time:%s', users[j], tim))
			...
			for _, v in pairs(url_tab) do
				...
				for l=1, #GPS do
					...
					if (not tonumber(GPS[l]['dir'])) or (not tonumber(GPS[l]['alt'])) or (not tonumber(GPS[l]['speed'])) then
						print(string.format('time : %s,key : ACTIVEUSER:%s,  imei : %s',os.date('%Y-%m-%d %H:%M:%S',i), tim, v['imei']))
					end
					...
					gps_info = string.format('%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s', 
								...
								MATH_FLOOR(tonumber(GPS[l]['alt'] or 0)), MATH_FLOOR(tonumber(GPS[l]['dir'] or -1)), MATH_FLOOR(tonumber(GPS[l]['speed']or 0)), v['tokencode'])
					local key = string.format('GPS:%s:%s', users[j], tim)
					local ok, ret = redis.cmd('bak_redis_w', 'sadd', key, gps_info)
					if not ok or not ret then
						--print(string.format('failed to set GPS -->imei: %s, time: %s to back redis', users[j], tim))
				...
				for l=1, #GSENSOR do
					...
					if not ok or not ret then
						--print(string.format('failed to set GSENSOR -->> imei: %s, time : %s to back redis', users[j], tim))
